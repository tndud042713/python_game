...
stage_size=stage.get_rect().size # 사이즈 정보가 필요함
...
balls.append({
    ...
    "to_x": 3, # 공의 축 이동방향, -3 이면 왼쪽으로, 3 이면 오른쪽으로
    "to_y": -6, # y축 이동방향,
...
while running:  
    ...
    for event in pygame.event.get():  
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_LEFT: # 캐릭터를 왼쪽으로
                ...
                weapon_y_pos = character_y_pos #무기가 플레이어 머리위로 나간다고 가정한다.
                weapons.append([weapon_x_pos, weapon_y_pos]) #weapons 에 정보가 들어감

        
        if event.type == pygame.KEYUP: # 키를 떼면
            if event.key == pygame.K_LEFT or event.key == pygame.K_RIGHT:
                character_to_x = 0 # 키를 떼면 움직이지 않도록 정의한다.
    ...
    # 가로 위치
        ...
        ## 캐릭터는 경계값을 만나면 멈추게 하면 되지만 공 같은 것은 벽면에서 튕겨나간다
        ...
        # 충돌 체크
            if weapon_rect.colliderect(ball_rect):
                ...
                if ball_img_idx < 3:
                    ...
                    # 나눠진 공 정보
                    small_ball_rect = ball_images[ball_img_idx + 1].get_rect() # idx + 1 을 해줘야 더 작은 공을 가져온다.
    ...
    ## 충돌 된 공 or 무기 없애기
    if ball_to_remove > -1:
        del balls[ball_to_remove] # 리스트에서 해당 위치에 있는 값이 지워짐
        ball_to_remove = -1
...
# 게임 오버 메시지
msg = game_font.render(game_result, True, (255,255,0)) #노란색 메시지
msg_rect = msg.get_rect(center=(int(screen_width / 2), int(screen_height / 2)))
screen.blit(msg, msg_rect)
pygame.display.update() # 업데이트 반영 해줘야 함

pygame.time.delay(2000) # 2초정도 후에 꺼지도록 함
